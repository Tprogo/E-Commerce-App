package com.example.e_comapplication.models.product_model


import android.os.Parcelable
import kotlinx.parcelize.Parcelize
import java.util.Collections
import java.util.Objects



data class ProductModel(
    val _links: Links,
    val attributes: List<Attribute>,
    val average_rating: String,
    val backordered: Boolean,
    val backorders: String,
    val backorders_allowed: Boolean,
    val button_text: String,
    val catalog_visibility: String,
    val categories: List<Category>,
    val cross_sell_ids: List<Int> = Collections.emptyList(),
    val date_created: String,
    val date_created_gmt: String,
    val date_modified: String,
    val date_modified_gmt: String,
    val date_on_sale_from: String ="",
    val date_on_sale_from_gmt: String ="",
    val date_on_sale_to: String = "",
    val date_on_sale_to_gmt: String = "",
    val default_attributes: List<DefaultAttribute>,
    val description: String,
    val dimensions: Dimensions,
    val download_expiry: Int,
    val download_limit: Int,
    val downloadable: Boolean,
    val downloads: List<String> = Collections.emptyList(),
    val external_url: String,
    val featured: Boolean,
    val grouped_products: List<Int> = Collections.emptyList(),
    val has_options: Boolean,
    val id: Int,
    val images: List<Image>,
    val low_stock_amount: String = "",
    val manage_stock: Boolean,
    val menu_order: Int,
    val meta_data: List<String> = Collections.emptyList(),
    val name: String,
    val on_sale: Boolean,
    val parent_id: Int,
    val permalink: String,
    val post_password: String,
    val price: String,
    val price_html: String,
    val purchasable: Boolean,
    val purchase_note: String,
    val rating_count: Int,
    val regular_price: String,
    val related_ids: List<Int>,
    val reviews_allowed: Boolean,
    val sale_price: String,
    val shipping_class: String,
    val shipping_class_id: Int,
    val shipping_required: Boolean,
    val shipping_taxable: Boolean,
    val short_description: String,
    val sku: String,
    val slug: String,
    val sold_individually: Boolean,
    val status: String,
    val stock_quantity: Int,
    val stock_status: String,
    val tags: List<String> = Collections.emptyList(),
    val tax_class: String,
    val tax_status: String,
    val total_sales: Int,
    val type: String,
    val upsell_ids: List<Int> = Collections.emptyList(),
    val variations: List<Int> = Collections.emptyList(),
    val virtual: Boolean,
    val weight: String,
)